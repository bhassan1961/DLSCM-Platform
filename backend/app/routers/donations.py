...
def list_donations(
    status: str | None = Query(None, description="Filter by donation status"),
...
def create_donation(donation: DonationCreate, db: Session = Depends(get_db)):
    ...
    if donation.quality_status not in VALID_QUALITY:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid quality_status. Must be one of: {VALID_QUALITY}",
        )
...
def update_donation_status(
    donation_id: int, update: DonationStatusUpdate, db: Session = Depends(get_db)
):
