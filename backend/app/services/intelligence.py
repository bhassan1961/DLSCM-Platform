...
NEWS_FEEDS = [
    {"name": "ReliefWeb Updates", "url": "https://reliefweb.int/updates/rss.xml", "source_type": "humanitarian"},
    {"name": "UNOCHA", "url": "https://www.unocha.org/rss", "source_type": "humanitarian"},
    {"name": "ICRC News", "url": "https://www.icrc.org/en/rss/news", "source_type": "humanitarian"},
]

ALL_NEWS_FEEDS = [
    {"id": "reliefweb", "name": "ReliefWeb", "url": "https://reliefweb.int/updates/rss.xml", "category": "humanitarian"},
    {"id": "unocha", "name": "UN OCHA", "url": "https://www.unocha.org/rss", "category": "humanitarian"},
    {"id": "icrc", "name": "ICRC", "url": "https://www.icrc.org/en/rss/news", "category": "humanitarian"},
    {"id": "who", "name": "WHO News", "url": "https://www.who.int/rss-feeds/news-english.xml", "category": "health"},
    {"id": "gdacs", "name": "GDACS", "url": "https://www.gdacs.org/xml/rss.xml", "category": "alerts"},
    ...
    {"id": "reuters", "name": "Reuters", "url": "https://www.reutersagency.com/feed/", "category": "news"},
    {"id": "guardian", "name": "The Guardian", "url": "https://www.theguardian.com/world/rss", "category": "news"},
    {"id": "ap", "name": "AP News", "url": "https://rsshub.app/apnews/topics/apf-topnews", "category": "news"},
]

DISASTER_TYPE_MAP = {
    "drought": ["drought", "food crisis", "famine", "water shortage", "crop failure"],
    "flood": ["flood", "flooding", "flash flood", "river overflow", "deluge", "inundation"],
    "earthquake": ["earthquake", "seismic", "tremor", "quake", "aftershock"],
    "cyclone": ["cyclone", "hurricane", "typhoon", "tropical storm", "storm surge"],
    "conflict": ["conflict", "armed", "military", "war", "displacement", "refugee", "IDP"],
    "epidemic": ["epidemic", "outbreak", "cholera", "ebola", "covid", "disease", "pandemic"],
}


def _relevance_score(text: str, disaster_name: str, country: str, disaster_type: str) -> float:
...
async def fetch_reliefweb_reports(
    ...
    params = {
        ...
        "limit": limit,
        "fields[include][]": [
            "title", "body-html", "url_alias", "source", "date",
            "country", "disaster_type", "format", "theme",
        ],
    }
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if resp.status_code == 200:
                ...
                for entry in data.get("data", []):
                    ...
                    body_text = re.sub(r"<[^>]+>", "", body_html)[:500] if body_html else ""
                    ...
                    format_name = formats[0].get("name", "Report") if formats else "Report"

                    full_text = f"{title} {body_text}"
                    score = _relevance_score(full_text, disaster_name, country, disaster_type)
...
async def fetch_live_news(feed_ids: list[str] | None = None, max_per_feed: int = 20) -> dict:
            ...
            if resp.status_code == 200:
                ...
                for item in root.findall(".//item"):
                    ...
                    items.append({
                        ...
                        "url": link,
                        "published_at": _parse_rss_date(pub_date),
                        "alert_level": alert_level.lower() if alert_level else "green",
...
async def get_crisis_intelligence(
    ...
    """
    reliefweb_task = fetch_reliefweb_reports(country, disaster_type, disaster_name, limit=30, days_back=days_back)
    ...
    results = await asyncio.gather(reliefweb_task, gdacs_task, news_task, return_exceptions=True)
    ...
    all_items.sort(key=lambda x: (x.get("relevance_score", 0), x.get("published_at", "")), reverse=True)
