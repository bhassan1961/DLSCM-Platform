...
def check_compliance(db: Session) -> dict:
    ...
    total_affected = (
        sum(d.affected_population for d in active_disasters) if active_disasters else 1
    )

    # Water: at least 15L/person/day
    water_stocks = db.query(Stock).join(Item).filter(Item.category == "water").all()
    ...
    sphere_standards.append(
        {
            ...
            else ("at_risk" if water_per_person >= 10 else "not_met"),
    ...
    shelter_stocks = db.query(Stock).join(Item).filter(Item.category == "shelter").all()
    ...
    sphere_standards.append(
        {
            ...
            "status": "met"
            if shelter_per_person >= 3.5
            else ("at_risk" if shelter_per_person >= 2.0 else "not_met"),
    ...
    food_stocks = db.query(Stock).join(Item).filter(Item.category == "food").all()
    ...
    sphere_standards.append(
        {
            ...
            "status": "met"
            if food_per_person >= 2100
            else ("at_risk" if food_per_person >= 1500 else "not_met"),
    ...
    avg_fulfillment = (
        round(sum(fulfillment_days_list) / len(fulfillment_days_list), 1)
        if fulfillment_days_list
        else 0.0
    )
    ...
    sphere_score = sum(
        30 if c["status"] == "met" else (15 if c["status"] == "at_risk" else 0)
        for c in sphere_standards
    )
