...
def detect_consumption_drift(daily_consumption: list[float], window: int = 7) -> dict:
    ...
    baseline_std = float(np.mean(ewma_std[:-window])) if np.mean(ewma_std[:-window]) > 0 else 1.0
...
def _diagnose_inventory(record: dict, features: list) -> str:
    ...
    reasons = []
    if understock > 0.5:
        reasons.append(f"critically understocked ({record.get('quantity')}/{record.get('min_required')})")
    if overstock > 2.0:
...
def _diagnose_shipment(record: dict, features: list) -> str:
    est, actual, delay, delay_ratio, items = features
    reasons = []
    if delay > 7:
    ...
    for r in records:
        ...
        if qty < min_req * 0.3 or qty > min_req * 5:
            anomalies.append({
                ...
                "reason": "critically low" if qty < min_req * 0.3 else "excessive overstock",
