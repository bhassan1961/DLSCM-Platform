...
def _build_disruption_model(supplier_data: list) -> dict:
    ...
    try:
        ...
        importances = dict(
            zip(
                feature_names,
                [round(float(v), 3) for v in gb_model.feature_importances_],
            )
        )
        ...
        for i, s in enumerate(supplier_data):
            ...
            ensemble_risk = round(
                (
                    risk_score * 0.6
                    + float(gb_proba[1] if len(gb_proba) > 1 else gb_proba[0])
                    * 100
                    * 0.4
                ),
                1,
...
def _identify_factors(feature_values, feature_names):
    ...
    for val, name in zip(feature_values, feature_names):
        if name in thresholds:
            ...
            if (direction == "high" and val > thresh) or (
                direction == "low" and val < thresh
            ):
                factors.append(
                    {"factor": name, "value": round(float(val), 3), "description": desc}
                )
    return factors
